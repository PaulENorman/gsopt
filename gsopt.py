...
def check_rate_limit(email: str) -> Tuple[bool, str]:
    ...
    _rate_limit_storage[email] = [
        ts for ts in _rate_limit_storage[email] if ts > window_start
    ]

    if len(_rate_limit_storage[email]) >= _RATE_LIMIT_MAX_REQUESTS:
        return (
            False,
            f"Rate limit exceeded: max {_RATE_LIMIT_MAX_REQUESTS} requests per {_RATE_LIMIT_WINDOW}s",
        )
...
def _ensure_optimizer_builder():
    ...
    if not _optimizer_builder_loaded:
        from skopt_bayes import (
            build_optimizer as build_skopt_optimizer,
            OptimizerSettings,
        )
...
def build_optimizer(
    settings: Any, existing_data: Optional[List[Dict[str, Any]]] = None
) -> Any:
...
def format_points_response(
    points: List[List[float]], param_names: List[str]
) -> List[Dict[str, Any]]:
    """
...
def init_optimization() -> Tuple[Any, int]:
    ...
    try:
        ...
        return jsonify(
            {
                "status": "error",
                "message": f"Failed to initialize optimization: {str(e)}",
            }
        ), 500
...
def continue_optimization() -> Tuple[Any, int]:
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": f"Failed to continue optimization: {str(e)}"}
        ), 500
...
def test_connection() -> Tuple[Any, int]:
    ...
    commit_sha = (
        os.environ.get("COMMIT_SHA") or os.environ.get("K_REVISION") or "development"
    )
...
def ping() -> Tuple[Any, int]:
    ...
    return jsonify(
        {"status": "success", "message": "Server is ready", "timestamp": time.time()}
    ), 200
    ...
    try:
        ...
        if not hasattr(skopt_opt, "get_result"):
            return jsonify(
                {
                    "status": "error",
                    "message": "Optimizer backend does not support get_result().",
                }
            ), 400
        ...
        if not res.x_iters or len(res.x_iters) == 0:
            return jsonify(
                {"status": "error", "message": "No data available to plot"}
            ), 400
        ...
        try:
            ...
            return jsonify(
                {
                    "status": "error",
                    "message": f"Error creating {plot_type}: {str(plot_err)}",
                }
            ), 500
        ...
        return jsonify({"status": "success", "plot_data": img_base64}), 200
